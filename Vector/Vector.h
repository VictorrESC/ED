...
public:
    ...
    // excecao 'out_of_range' se nao estiver. // O(1) // O(1)
    int& at(int k){
        if (k < 0 || k >= m_size){
    ...
    // Retorna uma referencia para o elemento na posicao k.
    // Essas funcoes nao verificam se o indice eh valido. // O(1) // O(1)
    ...
    // logo apos o ultimo elemento da lista. // tempo medio O(1)
